...
import { getPriceHistory, type HistorialPrecio } from "@/services/inventory";
...
export const HistorialPreciosModal = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const data = await getPriceHistory(productoId);
        // Orden por fecha desc (por si el backend aún no ordena)
        data.sort((a, b) => +new Date(b.created_at) - +new Date(a.created_at));
        setHistorial(data);
      } catch (e) {
        console.error("Historial precios:", e);
        setHistorial([]);
      } finally {
        setLoading(false);
      }
                ...
                <div key={item.id} className="p-4 border rounded-lg bg-card">
                  <div className="flex justify-between items-start mb-2">
                    <span className="text-sm text-muted-foreground">
                      {format(new Date(item.created_at), "dd 'de' MMMM, yyyy 'a las' HH:mm", {
                        locale: es,
                      })}
                    </span>
                  </div>
